...
public:
	long long minPath(int N, int K, vector <int> from, vector <int> to, vector <int> cost) {
        int visited[N][K+1][2];
        vector<pair<int, int> > neigh[1000];
        for(int i = 0;i<from.size();i++)
            neigh[from[i]].push_back(make_pair(to[i], cost[i]));
        for(int i = 0;i<N;i++)
            for(int j = 0;j<=K;j++)
                visited[i][j][0] = visited[i][j][1] = 1e9;
        int selfLooped[N];
        for(int i = 0;i<N;i++)
            selfLooped[i] = 0;
        for(int i = 0;i<from.size();i++){
            selfLooped[from[i]]++;
            if(from[i] == to[i])selfLooped[from[i]]--;
        }
        visited[0][0][0] = 0;
        ...
        while(!BFS.empty()){
            ...
            BFS.pop();
            int curN = x.first.first, curK = x.first.second;
            int curM = x.second;
            int curCost = visited[curN][curK][curM];
            for(int i = 0;i<neigh[curN].size();i++){
                int v = neigh[curN][i].first;
                int c = neigh[curN][i].second;
                if(visited[v][curK][0] > c+curCost)
                    visited[v][curK][0] = c+curCost, BFS.push(make_pair(make_pair(v, curK), 0));
                if(curK+1 <= K){
                    if(visited[v][curK+1][1] < c+curCost)
                        visited[v][curK+1][1] = c+curCost;
                }
            }
        }
	}
