...
inline node valid(int n1, int n2, int n3)
{
    node ret;
    ret.n = -1;

    if(n1>=0 && n2>=0 && n3>=0 && n1<=n && n2 <=n && n3<=n && abs(n1-n2)<=MAX_ALL && abs(n3-n2)<=MAX_ALL)
        ret.n = n1, ret.i = n2-n1+MAX_ALL, ret.j = n3-n2+MAX_ALL;
    return ret;

}
int dp[301][MAXX][MAXX];
int main()
{
    in_T
    {
...
//            for(int j = 0;j<MAXX;j++)
//            for(int k = 0;k<MAXX;k++)
//            if(j+i-MAX_ALL>=0 && j+i-MAX_ALL+k-MAX_ALL>=0 && j+i-MAX_ALL<=n && j+i-MAX_ALL+k-MAX_ALL<=n)
