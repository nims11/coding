...
//		for(int i=0;i<ratings.size();i++)
        ...
        int complete_grps = rat.size()/20;
        int partial_size = rat.size()%20;
        int R = complete_grps + (partial_size != 0);
        double cur = 0;
        for(int i=0;i<rat.size();i+=R)
        {
            bool flag = false;
            for(int j=i;j<rat.size() && j<i+R;j++)
                flag |= (rat[j] == e);
            if(i+R > rat.size())
            {
                int x = rat.size()-i;
                double sum = 0;
                for(int j=i;j<rat.size();j++)
                    sum += rat[j];
                sum /= x;
                if(flag)
                {
                    return (cur+e)/(1+rat.size()/R);
                }
                double expected = ((cur*(R-x))/R)/(rat.size()/R) + (((cur+sum)*x)/R)/(1 + rat.size()/R);
                return expected;
